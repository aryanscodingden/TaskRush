...
export default function FocusTimer() {
  ...
  if (finished) {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/5 backdrop-blur-[2px]">
        <BlurFade className="w-full max-w-sm">
          <div
            ...
            className="bg-white rounded-3xl border-2 border-zinc-100 p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)]"
          >
            <div className="text-center">
              <div className="text-sm font-medium text-zinc-400 uppercase tracking-wider mb-2">{taskTitle}</div>
              ...
              <div className="flex flex-col gap-2">
                <button
                  onClick={() => reset()}
                  className="w-full py-4 rounded-2xl bg-green-600 text-white font-bold hover:bg-green-700 transition-colors active:scale-[0.98]"
                >
                  Start Next Task
                </button>
                ...
                <button
                  onClick={() => reset()}
                  className="w-full py-4 rounded-2xl bg-zinc-50 text-zinc-500 font-bold hover:bg-zinc-100 transition-colors"
                >
                  Take a Break
                </button>
              </div>
      ...
      <BlurFade className="pointer-events-auto">
        ...
        >
          ...
          <div className="flex justify-center mb-8 relative">
             <div className="font-black text-7xl tracking-tighter text-zinc-900 tabular-nums">
          ...
          <div className="grid grid-cols-2 gap-4">
            {isRunning ? (
              <>
                <button
                  onClick={() => pause()}
                  className="py-4 rounded-2xl bg-red-50 text-red-600 font-bold hover:bg-red-100 transition-colors active:scale-95"
                >
                  Pause
                </button>
                <button
                  onClick={() => finish()}
                  className="py-4 rounded-2xl bg-green-600 text-white font-bold hover:bg-green-700 transition-colors shadow-lg shadow-green-200 active:scale-95"
                >
                  Finish
                </button>
              </>
          ...
          <div className="mt-6 text-center">
             <span className="text-xs font-bold text-zinc-300 uppercase tracking-widest">
                {takenMinutes}m / {expectedMinutes}m
             </span>
          </div>
        </div>
      </BlurFade>
