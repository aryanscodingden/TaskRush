...
const glassButtonVariants = cva("relative isolate inline-flex items-center justify-center cursor-pointer rounded-full transition-all appearance-none border-0 bg-transparent p-0 m-0 focus:outline-none", { variants: { size: { default: "text-base font-medium", sm: "text-sm font-medium", lg: "text-lg font-medium", icon: "h-10 w-10" } }, defaultVariants: { size: "default" } });
...
export const GlassButton = React.forwardRef<HTMLButtonElement, GlassButtonProps>(
  ({ className, children, size, contentClassName, onClick, ...props }, ref) => {
    const handleWrapperClick = (e: React.MouseEvent<HTMLDivElement>) => {
      // Only trigger if the click was on the wrapper directly (e.g. the shadow area),
      // NOT on the button or its children. This prevents double-clicks.
